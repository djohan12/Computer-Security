...
def generate_keys(key):
    p1 = ''.join([key[index-1] for index in pc1])
    c=[p1[:len(p1)//2]]
    d=[p1[len(p1)//2:]]
    for i in range(len(shifts)):
        c.append(shift(c[-1],shifts[i]))
        d.append(shift(d[-1],shifts[i]))
    
    keys=[c[i]+d[i] for i in range(len(c))][1:]
    for k in range(len(keys)):
        keys[k]=''.join([keys[k][index-1] for index in pc2])
    return keys


def f(key,right):
    Er = ''.join([right[i-1] for i in e])
    Ekey = xor(key,Er)
    
    groups = [Ekey[i:i+6] for i in range(0,48,6)]
    output=""
    ...
    for index, group in enumerate(groups):
        ...
        output+=bin(s[index+1][row][col])[2:].zfill(4)
    
    return ''.join([output[i-1] for i in p])
...
def main():
    ...
    left = permuted[:len(permuted)//2]
    right = permuted[len(permuted)//2:]
    ...
    for i in range(16):
        ...
        f_out=f(key,right)
        next_right = xor(left,f_out)
        print(f"Round {i+1:<2}:\n   Left : {next_left}\n   Right: {next_right}")
        
        left=next_left
        right=next_right
        
    final = next_right + next_left
    final = ''.join(final[i-1] for i in ip1)
    print(f"Final: {final}")
    plaintext = ''.join(chr(int(final[i:i+8], 2)) for i in range(0, len(final), 8))
